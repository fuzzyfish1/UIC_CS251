...
class DNAsequence {
 public:
  ...
  bool empty() { return strLetters.size() == 0; }
  // returns total number of letters in sequence
  int size() { return strLetters.size(); }

  // returns a specific character at an index of sequence
  char at(int i) { return strLetters.at(i); }
  ...
  // this is to check if a small segment of dna matches this one starting at an
  // index
  ...
  void clear() { this->strLetters.clear(); }
};

// loadDB, loads a database file with names and dna segment counts
void loadDB(string filePath, ourvector<DNAsequence>& personSegments,
...
void display(ourvector<personData>& fileData, DNAsequence& loadedDNA,
             ourvector<int>& segmentCount, ourvector<DNAsequence>& segments) {
  // print database
  if (fileData.size() > 0) {
...
// this thing uses already processed DNA and searches the peopleDB for a DNA
// match
void search(ourvector<int>& strCounts, ourvector<personData>& fileData,
...
void process(DNAsequence& DNA, ourvector<DNAsequence>& strs,
             ourvector<int>& data) {
  // check prerequisites
  if (strs.size() == 0) {
    cout << "No database loaded." << endl;
...
void countAll(DNAsequence& DNA, ourvector<int>& strCounts,
  ...
  // for every DNA segment we find, record its existence and add 1 to existing
  // things
